...
def display_table(list):
        ...
        for count, row in enumerate(sortedlist):
                ...
                if GCTLs.get(row['ANALYTPARAM'].lower()) != None:
                        lowerlevel = GCTLs.get(row['ANALYTPARAM'].lower())
                else:
                        lowerlevel = ''
                if NADCs.get(row['ANALYTPARAM'].lower()) != None:
                        upperlevel = NADCs.get(row['ANALYTPARAM'].lower())
                else:
                        upperlevel = ''
                if   GCTLs.get(row['ANALYTPARAM'].lower()) != None:        
                        if   row['CONCUNITS'] == 'UGL' and float(row['CONC']) > float(GCTLs.get(row['ANALYTPARAM'].lower())):
                                lower_Exceedance = 'GCTL_Exceedance'
                              
                                
                        elif row['CONCUNITS'] == 'MGL' and float(row['CONC'])*1000 > float(GCTLs.get(row['ANALYTPARAM'].lower())):
                                lower_Exceedance = 'GCTL_Exceedance'
                ...
                if   NADCs.get(row['ANALYTPARAM'].lower()) != None:
                        if    row['CONCUNITS'] == 'UGL' and float(row['CONC']) > float(NADCs.get(row['ANALYTPARAM'].lower())):
                                upper_Exceedance = 'NADC_Exceedance'
                               
                                
                        elif  row['CONCUNITS'] == 'MGL' and float(row['CONC'])*1000 > float(NADCs.get(row['ANALYTPARAM'].lower())):
                                upper_Exceedance = 'NADC_Exceedance'
                ...
                print("{:10}{:<20}{:<25}{:<39}{:>7}{:^5}{:<10}{:>10}{:>10}{:^25}{:^25}{:^25}".format \
                      (row['SITEID'],row['SAMPLOC'], row['SampDate'], row['ANALYTPARAM'], row['CONC'], row['QAQUAL'],\
                       row['CONCUNITS'], lowerlevel, upperlevel, Detection, lower_Exceedance, upper_Exceedance))
...
with fileinput.input(sys.argv[1:]) as fileset:
	...
	for count, row in enumerate(reader):
                ...
                if GCTLs.get(row['ANALYTPARAM'].lower()) != None:
                        GCTLno = GCTLs.get(row['ANALYTPARAM'].lower())
                else:
                        GCTLno = ''
                if NADCs.get(row['ANALYTPARAM'].lower()) != None:
                        NADCno = NADCs.get(row['ANALYTPARAM'].lower())
                else:
                        NADCno = ''
...
#                print(fileinput.filename(),fileinput.fileno(),fileinput.filelineno(), fileinput.lineno(), fileinput.isfirstline(), row['SAMPLOC'])
